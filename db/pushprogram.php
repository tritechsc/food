...
//include 'db.php'; include 'error.php'; 
 ...
 if (!($connection = mysqli_pconnect($host,$user, $pass))) die("Could not connect to database");
	mysql_select_db("ttsc", $connection);

	$hostname = gethostbyaddr($_SERVER['REMOTE_ADDR']);
//echo $hostname;
	
	$result = mysqli_query ("SELECT CURDATE();", $connection);
	$row = mysql_fetch_row($result);$date = $row[0];
   
	$result = mysqli_query ("SELECT CURTIME();", $connection);
